...
function getOpenState() {
  const open = new Set();
  document
    .querySelectorAll(".product-body.open, .phase-body.open, .task-body.open")
...
function updateProgressBar(productId) {
  const product = roadmapData.products.find((p) => p.id === productId);
  const progress = calculateProgress(product);
  const fill = document.querySelector(`#product-${productId} .progress-fill`);
...
function deleteProduct(productId) {
  ...
  roadmapData.products = roadmapData.products.filter((p) => p.id !== productId);
  renderRoadmap();
  saveData();
}
...
function showAddTaskModal(productId, phaseId) {
  createModal(
    ...
    (values) => {
      ...
      document.getElementById(`body-${productId}`)?.classList.add("open");
      document
        .getElementById(`phasebody-${productId}-${phaseId}`)
        ?.classList.add("open");
      saveData();
    },
  );
  ...
  createModal(
    ...
    (values) => {
      ...
      document
        ...
        ?.classList.add("open");
      document
        .getElementById(`taskbody-${productId}-${phaseId}-${taskId}`)
        ?.classList.add("open");
      saveData();
    },
  );
...
function updateLastUpdated() {
  ...
  if (roadmapData.lastUpdated) {
    el.textContent = `LAST UPDATED: ${new Date(roadmapData.lastUpdated)
      .toLocaleString()
      .toUpperCase()}`;
  }
}
